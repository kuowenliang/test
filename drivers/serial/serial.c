...
/* Multi serial device functions (specify baudrate)*/
#define DECLARE_ESERIAL_FUNCTIONS_ENV(port, baudrate) \
    int  eserial##port##_init (void) {\
	int clock_divisor; \
	clock_divisor = calc_divisor_by_env(serial_ports[port-1], baudrate); \
	NS16550_init(serial_ports[port-1], clock_divisor); \
	return(0);}\
    void eserial##port##_setbrg (void) {\
	serial_setbrg_dev_by_env(port, baudrate);}\
    int  eserial##port##_getc (void) {\
	return serial_getc_dev(port);}\
    int  eserial##port##_tstc (void) {\
	return serial_tstc_dev(port);}\
...
static int calc_divisor_by_env (NS16550_t port, char* item)
{
	unsigned long baudrate = (unsigned long)simple_strtoul(getenv(item), NULL, 10);
#ifdef CONFIG_OMAP1510
	/* If can't cleanly clock 115200 set div to 1 */
	if ((CONFIG_SYS_NS16550_CLK == 12000000) && (baudrate == 115200)) {
		port->osc_12m_sel = OSC_12M_SEL;	/* enable 6.5 * divisor */
		return (1);				/* return 1 for base divisor */
	}
	port->osc_12m_sel = 0;			/* clear if previsouly set */
...
#ifdef CONFIG_OMAP1610
	/* If can't cleanly clock 115200 set div to 1 */
	if ((CONFIG_SYS_NS16550_CLK == 48000000) && (baudrate == 115200)) {
		return (26);		/* return 26 for base divisor */
	}
#endif
...
#define MODE_X_DIV 16
	...
	/* Compute divisor value. Normally, we should simply return:
	 *   CONFIG_SYS_NS16550_CLK) / MODE_X_DIV / baudrate
	 * but we need to round that value by adding 0.5.
	 * Rounding is especially important at high baud rates.
	 */
...
#if 0
static int calc_divisor_by_baudrate (NS16550_t port, unsigned long baudrate)
{
#ifdef CONFIG_OMAP1510
	/* If can't cleanly clock 115200 set div to 1 */
	if ((CONFIG_SYS_NS16550_CLK == 12000000) && (baudrate == 115200)) {
...
void 
_serial_setbrg_by_env (const int port, char* item)
{
	int clock_divisor;

	clock_divisor = calc_divisor_by_env(PORT, item);
	NS16550_reinit(PORT, clock_divisor);
}

#if 0
void 
_serial_setbrg_by_baudrate (const int port, unsigned long baudrate)
{
	int clock_divisor;

	clock_divisor = calc_divisor_by_baudrate(PORT, baudrate);
	NS16550_reinit(PORT, clock_divisor);
}
#endif
...
static inline void
serial_setbrg_dev_by_env(unsigned int dev_index, char* item)
{
	_serial_setbrg_by_env(dev_index, item);
}
#if 0
static inline void
serial_setbrg_dev_by_baudrate(unsigned int dev_index, unsigned long baudrate)
{
	_serial_setbrg_by_baudrate(dev_index, baudrate);
}
#endif
#else
void
...
DECLARE_ESERIAL_FUNCTIONS_ENV(2, CONFIG_BAUDRATE_1_ITEM);
#else
...
DECLARE_ESERIAL_FUNCTIONS_ENV(3, CONFIG_BAUDRATE_2_ITEM);
#else
...
DECLARE_ESERIAL_FUNCTIONS_ENV(4, CONFIG_BAUDRATE_3_ITEM);
#else
