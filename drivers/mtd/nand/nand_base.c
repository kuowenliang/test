...
/*
 * Get the flash ONFI parameter page
 */
void nand_read_parameter(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	/* Send the command for reading the ONFI parameter page */
	chip->cmdfunc(mtd, NAND_CMD_PARAMETER, 0x00, -1);

	/* Read the ONFI parameter page */
	for(i=0; i<67; i++){
		buff[i] = chip->read_byte(mtd);
	}
	MTDDEBUG (MTD_DEBUG_LEVEL0, "Date code: 0x%02x%02x\n", buff[65], buff[66]);
...
int nand_scan_ident(struct mtd_info *mtd, int maxchips)
{
	...
	/* Read the flash ONFI parameter page */
	nand_read_parameter(mtd, chip);
