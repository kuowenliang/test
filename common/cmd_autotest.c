...
//	i = 0;
	...
	for(cnt=0;cnt<maxCount;cnt++){
		if((cnt%0x10000)==0) {
			printf("\rVerify DDR free area - 0x%08X                ", (unsigned int)ddrSr);
		//	i++;
		//	if(i >= 60) {
		//		i = 0;
		//		printf("\r\n");
...
#else
	retVal |= DRVfnAudio_AIC3104SendData(LINE2_2_LADC_CTRL, 0x0f);
	retVal |= DRVfnAudio_AIC3104SendData(LINE2_2_RADC_CTRL, 0xf0);
	retVal |= DRVfnAudio_AIC3104SendData(PGAL_2_LLOPM_VOL, 0x80);
	retVal |= DRVfnAudio_AIC3104SendData(LLOPM_CTRL, 0x0b);
