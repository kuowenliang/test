...
static ulong load_serial_ymodem (ulong offset, int* img_size);
#endif
...
static ulong load_serial_bin (ulong offset, int* img_size);
...
int serial_loadimg (char* mode, ulong address)
{
	ulong offset = 0;
	ulong addr;
	int load_baudrate;
	...
	load_baudrate = gd->baudrate;

	if(mode && (strcmp(mode,"loady")==0)){
		printf ("## Ready for binary (ymodem) download "
			"to 0x%08lX at %d bps...\n",
			offset,
			load_baudrate);

		addr = load_serial_ymodem (offset, &size);
		...
		printf ("## Ready for binary (kermit) download "
			...
			offset,
			load_baudrate);
		addr = load_serial_bin (offset, &size);
